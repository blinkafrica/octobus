...
This section describes how Authentication is performed in Octobus.

## Introduction

The Authentication module in Octobus exists for 2 main reasons:
...
The **RedisStore** module **manages user sessions** and has 4 main utility functions through which token-related actions for authentication are performed in Octobus. They are discussed below:
...
Next, we create and Octobus's `RedisStore` instance as shown below

```js
// redis.store.ts

import { RedisStore } from "@blinkclaud/octobus";
