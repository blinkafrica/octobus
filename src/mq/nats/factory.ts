...
export class StreamFactory {
  ...
  /**
   * Create a stream factory that can be used to create streams
   * @param arg either a nats URL or a list of URLs separated by comma, or a NatsConnection object
   * @param opts nats connection options
   * @returns a stream factory
   */
  ...
  /**
   * Creates a stream with the given name and configuration
   * @param name name of the stream
   * @param conf configuration of the stream
   * @returns a stream
   */
  async stream<T>(name: string, conf: StreamConfig) {
    ...
    const parsedConf = {
      ...
      ...(conf.stream_type === 'log' && { max_age: ms(conf.retention_period) * 1_000_000 }),
      ...(conf.stream_type === 'broadcast' && { max_msgs: conf.buffer_size }),
